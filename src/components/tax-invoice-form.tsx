...
export function TaxInvoiceForm({ jobId: jobIdProp, editDocId: editDocIdProp }: { jobId: string | null, editDocId: string | null }) {
  ...
  const [showTaxInfoAlert, setShowTaxInfoAlert] = useState(false);
  ...
  const handleGoToEditCustomer = () => {
    if (currentCustomer) {
      router.push(`/app/management/customers?editPhone=${currentCustomer.phone}`);
    }
  };

  const handleCancelCreation = () => {
    setShowTaxInfoAlert(false);
    router.back();
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <Form {...form}>
        <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
          ...
          <Dialog open={showDuplicateDialog} onOpenChange={setShowDuplicateDialog}>
            <DialogContent className="sm:max-w-md">
              ...
              <DialogFooter className="gap-2">
                ...
                <Button variant="destructive" onClick={handleCancelExistingAndSave} disabled={isProcessing}>
          ...
          <AlertDialog open={showTaxInfoAlert} onOpenChange={setShowTaxInfoAlert}>
            <AlertDialogContent className="sm:max-w-md">
              ...
              <AlertDialogFooter className="flex flex-col sm:flex-row gap-2 mt-4">
                <Button variant="outline" className="w-full sm:w-auto" onClick={handleCancelCreation}>
                  <XCircle className="mr-2 h-4 w-4" /> ยกเลิกการสร้าง
                </Button>
                <Button className="w-full sm:w-auto bg-primary" onClick={handleGoToEditCustomer}>
                  <UserCog className="mr-2 h-4 w-4" /> ไปแก้ไขข้อมูลลูกค้า
                </Button>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
