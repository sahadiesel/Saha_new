      ...
      <Tabs value={tab} onValueChange={(v) => replaceTab(v === "new" ? "new" : "list")} className="w-full">
        <div className={cn("flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8")}>
          <div className="grid gap-1">
            <h1 className="font-headline text-3xl md:text-4xl font-bold tracking-tighter">ใบเพิ่มหนี้</h1>
            <p className="text-muted-foreground">จัดการข้อมูลใบเพิ่มหนี้ (อ้างอิงใบกำกับภาษี)</p>
          </div>
        ...
        <TabsContent value="new" className="mt-0">
          <DebitNoteForm onCancel={() => replaceTab("list")} />
        </TabsContent>
      </Tabs>
    </div>
  );
